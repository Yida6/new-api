...
| 用户额度、预扣费和结算 | `【已有/已验证】` | 测试、生产各完成 1 个 5 秒 480P 成功任务；异常场景自动化测试确认预扣退款，本地余额不足会在请求上游前拒绝 | 成功任务扣费、差额结算及失败退款代码路径已验证；真实方舟失败账单仍待生产对账 |
...
- [ ] 尚未验证图生视频、相同幂等键重复提交、成本熔断、备用渠道和方舟账单三方对账；失败/超时退款已通过本地自动化故障注入验证，但真实方舟故障账单仍待生产验收。
...
- [ ] 已确认火山方舟账号允许当前调用方式
- [ ] 在线支付保持关闭：支付合规条款未确认、所有网关凭据为空，支付信息接口返回所有在线支付开关为 `false`
