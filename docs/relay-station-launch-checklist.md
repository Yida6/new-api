...
- [ ] 尚未验证图生视频、成本熔断、备用渠道和方舟账单三方对账；客户端持久幂等已于 8-20 修复上线并通过测试渠道真实重放验收，详见 `docs/production-ark-failure-acceptance-2026-08-20.md`。
...
- [ ] `【自动化已通过，生产部分验收】` 8-20 仅使用 `test` 组测试渠道完成真实版权限制失败：单次预扣 433,851 quota 后全额退款，余额恢复、并发释放、渠道保持启用且密钥零泄露；真实超时及不可安全制造的 401、429、500/503、余额不足仍保留自动化覆盖，详见 `docs/production-ark-failure-acceptance-2026-08-20.md` `P0`
...
- [x] 记录当前数据库备份和应用版本（8-20 最新：`newapi-20260820T045025Z.sql.gz` / `new-api-central-asia:e1b5431c`）`P0`
...
| 固定部署源码版本 | 当前生产 Commit `e1b5431c`；首个部署候选基线标签为 `deploy-central-asia-20260817-rc1` |
| 生产镜像 | `new-api-central-asia:e1b5431c`；`sha256:d07bd953a34695328f621fbe2960ade1b5c6cf12941b1176c9a33cedc8f9a4b7` |
