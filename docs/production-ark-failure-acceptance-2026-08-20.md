...
- `arkcli` SSO 已重新认证。
- 8 月 20 日下午再次使用 `arkcli billing list --start 2026-08 --interval day --day 2026-08-20` 查询；账号仍未开通拆分账单能力，接口返回“当前账号未开通拆分账单能力，无数据”。
- 火山账单为 T+1，测试当天不能取得该任务的最终结算明细。
- 因此本报告确认本地退款闭环通过，但不将方舟侧最终结算金额标记为已核对。

## 视频输入、分辨率与音频选项补充验收

8 月 20 日仅使用 `test` 组渠道补做视频输入验收。输入素材取自同一测试渠道当天成功任务的短期方舟 TOS 结果 URL；提交前确认 URL 可读取（HTTP 206），完整 URL、测试 Token 和渠道密钥均未写入报告或命令输出。

首次请求真实触发方舟 HTTP 400：视频条目缺少 `role=reference_video`。系统在上游创建任务前退回全部 1,323,660 quota，未产生任务记录。补齐角色后请求进入 `IN_PROGRESS`，方舟将其识别为视频编辑，并以终态失败返回该模式必须使用 `ratio=adaptive`、`duration=-1`；任务最终 `quota=0`、`concurrency_released=true`、`token_delta_pending=0`，预扣再次全额退回。

该真实返回暴露出中转层不接受方舟视频编辑时长哨兵 `metadata.duration=-1`。提交 `7356ca53` 做了定向兼容：仅当请求包含 `video_url` 且 `ratio=adaptive` 时允许 metadata 中的 `-1`；顶层 `-1`、无视频输入或非 adaptive 比例仍在本地拒绝。以下验证均通过：

```text
go test ./relay/channel/task/doubao -count=1
go test ./service ./relay ./controller -count=1
go build ./...
```

生产镜像 `new-api-central-asia:7356ca53` 上线前备份为 `newapi-20260820T051338Z.sql.gz`，容器健康检查和公网冒烟检查通过。修复后视频输入任务 `task_bIC…4p0d` 验收结果如下：

| 项目 | 结果 |
|---|---|
| 路由 | `group=test`，`channel_id=4` |
| 输入 | 同测试渠道成功视频，`role=reference_video` |
| 请求选项 | 720P、`ratio=adaptive`、`duration=-1`、关闭音频 |
| 状态 | `NOT_START` → `IN_PROGRESS` → `SUCCESS` |
| 方舟实际结果 | 720P、5 秒、9:16、无音频，usage 216,900 tokens |
| 系统预扣 | 1,323,660 quota |
| 完成后差额退款 | 693,761 quota |
| 最终扣费 | 629,899 quota |
| 用户余额 | 498,617,302 → 497,987,403，差额为 629,899 |
| 任务收敛 | `concurrency_released=true`，`token_delta_pending=0` |
| 渠道 | ID `4` 保持启用 |
| 成品下载 | HTTP 200，4,423,508 字节，MP4 `ftypisom` 文件头有效 |

同一测试渠道已有 720P、5 秒、带音频成功任务作为音频开启对照；本次成功任务关闭音频并使用视频输入，因此视频输入、其他分辨率和音频开关均已有真实任务及系统结算证据。新任务完成后再次抽检：数据库 logs 和应用日志对完整渠道密钥、完整测试 Token 均为 0 命中，数据库中 `Authorization` 也为 0 命中。

上述任务返回的方舟 usage、系统任务日志与用户余额扣费三者已能相互核对；但 usage 不是火山计费中心的最终 CNY 财务明细。账号未开通拆分账单且当日数据尚未 T+1 出账，因此三方财务对账仍保持未完成状态。

## 结论

真实上游拒绝/任务失败、每任务单次退款、客户端持久幂等和视频输入成功结算均已通过生产验收；同键同请求不会再创建第二个方舟任务或再次扣费。真实超时和不可安全制造的上游故障仍保留自动化证据，等待自然故障或独立隔离环境补充。方舟计费中心最终财务明细因账号能力与 T+1 时效仍待补齐。
