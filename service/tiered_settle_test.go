...
func (*recordingBillingSettler) Settle(int64) error { return nil }

func (*recordingBillingSettler) Refund(*gin.Context) {}

func (*recordingBillingSettler) NeedsRefund() bool { return false }

func (s *recordingBillingSettler) GetPreConsumedQuota() int64 {
	return s.preConsumedQuota
}

func (s *recordingBillingSettler) Reserve(targetQuota int64) error {
...
func TestPrepareTieredBillingForSelectedGroupUpdatesReservation(t *testing.T) {
	...
	assert.Equal(t, int64(100_000), billing.preConsumedQuota)
	assert.Equal(t, int64(100_000), relayInfo.FinalPreConsumedQuota)
	assert.Equal(t, 0.20, relayInfo.TieredBillingSnapshot.GroupRatio)
	assert.Equal(t, int64(100_000), relayInfo.TieredBillingSnapshot.EstimatedQuotaAfterGroup)
}
...
func TestPrepareTieredBillingForSelectedGroupPaidToFreeKeepsFreeModelFalse(t *testing.T) {
	...
	assert.Equal(t, int64(50_000), relayInfo.FinalPreConsumedQuota)
}
...
func TestPrepareTieredBillingForSelectedGroupTopUpArrearsAllowsNegativeBalance(t *testing.T) {
	...
	assert.Equal(t, int64(100_000), session.GetPreConsumedQuota())
	assert.Equal(t, int64(100_000), relayInfo.FinalPreConsumedQuota)
	assert.Equal(t, int64(100_000), relayInfo.TieredBillingSnapshot.EstimatedQuotaAfterGroup)
	userQuota, err := model.GetUserQuota(userID, false)
	require.NoError(t, err)
...
func TestTryTieredSettleUsesFinalGroupAfterRetry(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, int64(tt.wantQuota), quota)
			assert.Equal(t, tt.finalGroupRatio, relayInfo.TieredBillingSnapshot.GroupRatio)
			assert.Equal(t, int64(tt.wantQuota), relayInfo.TieredBillingSnapshot.EstimatedQuotaAfterGroup)
		})
	}
}
