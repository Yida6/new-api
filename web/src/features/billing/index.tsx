...
import { isBillableLog, sumInputTokens, sumOutputTokens } from './lib/format'
import {
  BILLING_FETCH_BATCH,
  BILLING_MAX_ROWS,
...
export function Billing() {
  ...
  const load = useCallback(
    async (next: BillingFilters) => {
      ...
      try {
        ...
        // Fetch all matching records (bounded) for client-side aggregation
        // and pagination. The /stat endpoint only reports rpm/tpm for the
        // last 60 seconds, so request/token totals cannot come from it.
  ...
  const stats: BillingStatsData | null = useMemo(
    () => ({
      ...
      inputTokens: sumInputTokens(logs),
      outputTokens: sumOutputTokens(logs),
    }),
    [netQuota, logs]
  )
