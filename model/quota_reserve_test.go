...
func TestRedisBatchReserveNeverFallsBackToStaleDatabaseBalance(t *testing.T) {
	...
	assert.Equal(t, 2, getUserQuotaFromDB(t, user.Id), "授权扣减必须立即直写落账，不能只入队")
	...
	assert.Equal(t, 2, getTokenFromDB(t, token.Id).RemainQuota, "有限 Token 授权扣减必须立即直写落账，不能只入队")
	assert.Equal(t, 7, getTokenFromDB(t, token.Id).UsedQuota)
	reserved, err = TryReserveTokenQuota(token.Id, token.Key, 3, false)
	require.NoError(t, err)
	assert.False(t, reserved)
	assert.Equal(t, 2, getTokenFromDB(t, token.Id).RemainQuota, "余额不足时 DB 不得扣成负数")
	...
	assert.Equal(t, 9, reloadedToken.RemainQuota+reloadedToken.UsedQuota, "remain+used 不变量保持不变")
}
...
func TestSynchronousReserveCompensatesCacheWhenPersistenceFails(t *testing.T) {
	...
	require.NoError(t, cacheErr)
	assert.Equal(t, 12, cachedToken.RemainQuota, "授权失败后缓存必须补偿回原值")
